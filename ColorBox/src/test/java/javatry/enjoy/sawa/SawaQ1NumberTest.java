...
public class SawaQ1NumberTest extends ColorBoxTestCase {
    ...
    // done sawa @paramのcontentが警告でてる！なにを受け取るのか、右側に日本語でいいので説明をしてあげましょう！ by hakiba (2017/05/15)
    /**
            ...
            if ("blue".equals(colorBox.getColor().getColorName())) {
                for (BoxSpace boxSpace : spaceList) {
                    ...
                    if (contents instanceof Map<?, ?>) {
                        int valueMax = 0;
                        // done sawa 製品がない状態を示したいなら空文字ではなく, nullを使う方が自然かな。空文字だと""っていう文字列が存在してしまうので。 by hakiba (2017/05/15)
                        String product = null;
