...
public class SawaQ2DateTest extends ColorBoxTestCase {
    ...
    public void test_convert() {
        // TODO done sawa colorBoxListはforの引数でしか呼ばれないので、変数に出さずに書いてみよう by yuki.wakisaka (2017/04/30)
        ...
        for (ColorBox colorBox : getColorBoxList()) {
            ...
            for (BoxSpace boxSpace : spaceList) {
                ...
                if (contents instanceof LocalDate) {
                    ...
                    // TODO done sawa FormatterはLocalDateでもLocalDateTimeでも使いまわすので、外で定義しちゃうのがいいかな。
                    // 今回の出力形式を定義するという意味では、1行目とかでもいいくらい。 by yuki.wakisaka (2017/05/01)
                    // TODO done sawa log(...) はfor文の外に出しておこう。処理の切り分け。 by yuki.wakisaka (2017/05/01)
    ...
    public void test_weekOfDay() {
        ...
        // TODO done ↑ log(...)はfor文の外に出しちゃおう、の意も含む！ by yuki.wakisaka (2017/05/01)
    ...
    public void test_weekOfDayOf2017Newcomer() {
        ...
        // TODO done これをStringではなくDayOfWeekで持てば、比較するときも楽 by yuki.wakisaka (2017/05/01)
        ...
        if (entryWeek.equals(DayOfWeek.SATURDAY) || entryWeek.equals(DayOfWeek.SUNDAY)) {
            // TODO done sawa ここでentryWeekに月曜を再代入してlogに出すほうが、出力形式にブレが減る by yuki.wakisaka (2017/05/01)
    ...
    public void test_diffDay() {
        ...
        for (ColorBox colorBox : colorBoxList) {
            if (colorBox.getColor().getColorName().equals("yellow")) {
                ...
                // TODO done sawa 1つの場合のlogも出そう。& 3つ以上の場合は？ by yuki.wakisaka (2017/05/01)
                // TODO [コメント] 日付が二つの場合以外は問題の要件を満たさないので、elseを通るようにしました！ by sawa (2017/05/01)
                if (date.size() == 2) {
